...
public class SuffererOutModel implements SuffererOutContract.iModel {
    ...
    @Override
    public void getPostReviewData(int doctorId, String sessionId, int sickCircleId, String content, iSuffererOutCallBack callBack) {
        RetrofitManager.getInstance().create(ApiServers.class)
                .getPostReview(doctorId,sessionId,sickCircleId,content)
                .compose(CommonSchedulers.io2main())
                .subscribe(new CommonObserver<PostReviewBean>() {
                    @Override
                    public void onNext(PostReviewBean postReviewBean) {
                        callBack.onPostReviewSuccess(postReviewBean);
                    }

                    @Override
                    public void onError(Throwable e) {
                        callBack.onPostReviewFailure(e);
                    }
                });
    }
}
